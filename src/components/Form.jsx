...
const useStyles = makeStyles({
  root: {
    ...
    marginBottom: 16,
    padding: 16,
    boxShadow: "0px 3px 5px -1px rbga(0,0,0,0.2), 0px 6px 10px 0px rgba(0,0,0,0.14), 0px 1px 18px 0px rgba(0,0,0,0.12)"
  },
  button:{
    marginTop:16
  }
...
const Form = ({title, setTitle, addItem, editItem, edit, error, setError }) =>{
  ...
  const handleChange =(event) =>{
    ...
    setTitle(title);
    if(title.length === 0){
      setError("Por Favor coloque o titulo");
    }else{
      setError("");
    }
  }
